...
class ObstacleAvoider(Node):
    ...
    def __init__(self):
        ...
        self.get_help_from_right_hand = False
        self.previous_move = 'forward' # left or right or forward
        self.state = 1
    ...
    def scan_callback(self, msg):
        ...
        if self.tf_buffer.can_transform(target_frame, source_frame, rclpy.time.Time()):
            ...
            if self.state == 2:
                if right_distance < (self.min_distance - self.telorance):
                    self.turn_left()

                elif right_distance > (self.min_distance + self.telorance):
                    self.turn_right()
                else:
                    self.move_forward()
